      ...
      <ThreeColWrapper sidebarMargin={false}>
        ...
        {response && !error && (
          <>
            ...
            <PlaylistDurationCard
              response={response}
              start={parseInt(startCounterRef.current?.value as string)}
              end={parseInt(endCounterRef.current?.value as string)}
            />
          </>
        )}
      </ThreeColWrapper>
...
export default dynamic(() => Promise.resolve(PlaylistDuration), { ssr: false });
